...
Cloudflare cron (hourly fallback)   ─┤
...
Trigger.dev runs the primary schedule because it retries each step and keeps a
run history you can point at. The Cloudflare Worker is the shared store the
sweep writes and the extension reads, with an hourly fallback sweep so the store
can refill itself if the schedule is down. `chrome.alarms` is the local path and
is what publishes the artifact you currently have open.
...
**A finding that stops being true is retired — but only on a clean sweep.** If
any source degraded, the sweep cannot know what it missed, so nothing is
retired: erasing real findings on every hiccup would be worse than carrying a
stale one for a few minutes.
